...
ax0.set_title('Catégorie observée')
ax1.set_title(f'Catégorie prédite par SVM - accuracy {int(100 * svm_accuracy)}%')
ax2.set_title(f'Catégorie prédite par KNN - accuracy {int(100 * knn_accuracy)}%')
for ax in [ax0, ax1, ax2]:
    ax.legend(bbox_to_anchor=(1.02, 1), loc='upper left', borderaxespad=0)
    ax.axis('off')
plt.suptitle('Projection TSNE du jeu de test')
plt.tight_layout()
plt.show()
